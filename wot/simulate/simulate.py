...
def multivariate_normal_evolving_mixture(means, covs, p=None, size=1):
    ...
    covs = numpy.asarray(covs, dtype=numpy.float64)
    if covs.ndim == 1:
        covs = numpy.ascontiguousarray([ [ c * numpy.identity(n) ] * k for c in covs ])
    elif covs.ndim == 2:
        covs = numpy.ascontiguousarray([ [ c * numpy.identity(n) for c in cov ] for cov in covs ])
    elif covs.ndim == 3:
        covs = numpy.ascontiguousarray([ [ numpy.diag(c) for c in cov ] for cov in covs])
    else:
        covs = numpy.ascontiguousarray(covs, dtype=numpy.float64)
    ...
    if p.ndim < 2:
        p = numpy.ascontiguousarray([ p ] * t, dtype=numpy.float64)
    else:
        p = numpy.ascontiguousarray(p, dtype=numpy.float64)

    if isinstance(size, int):
        size = [ size ] * t
    size = numpy.ascontiguousarray(size, dtype=numpy.int64)
